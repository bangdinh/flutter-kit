---
name: flutter-arch
description: Use when writing, reviewing or restructuring Dart/Flutter code in flutter-kit or in an app built on it — where a file belongs, layer dependency rules, Riverpod 3.x providers, freezed models, error/Result contract, theming, what belongs in the kit vs in the app. Trigger khi "thêm feature", "tạo provider", "review code Flutter", "kiến trúc Flutter", "đặt file ở đâu", "sửa theme", "gọi API", "nên để ở kit hay app".
---
...
the app name, an endpoint path, or product wording — it's an app rule. Add an **extension point**
to the kit instead (a contract + a provider the app overrides), never an app-specific branch.
...
Write the failing test first (red → green → refactor). A new kit extension point needs a test
proving the default *and* that an app override wins — see `test/network/api_client_test.dart`.
